...
class Cart{
    ...
    replaceItemCount(skuId,newCount){
        ...
        if(!oldItem){
            console.error('异常情况,更新CartItems找不到对应的数据')
            return
        }
        if(newCount < 1){
    ...
    async getAllSkuFromServer() {
        ...
        const serverData = await Sku.getSkuById(skuIds)
        this.refreshByServerData(serverData)
        this.refreshStorage()
        return this._getCartData()
    }
    ...
    getSkuCountBySkuId(skuId){
        ...
        if(!item){
            console.error('订单里寻找CartItem时不应当出现找不到的情况')
        }
        return item.count
    }

    refreshByServerData(serverData){
        const cartData = this._getCartData()
        cartData.items.forEach(item=>{
            this._setLatestCatItem(item,serverData)
        })
    }

    _setLatestCatItem(item,serverData){
        let removed = true
    ...
    removeItem(skuId){
        ...
        carData.items.splice(oldItemIndex,1)
        this.refreshStorage()
    }

    _findEqualItemIndex(skuId){
       return  this._getCartData().items.findIndex(item=> { return item.skuId === skuId})
    }
